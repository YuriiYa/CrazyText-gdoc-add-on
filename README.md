...
**IMPORTANT**: add your test user to test users list.
...
8. Return to the Cloud Console and open **Google Workspace Marketplace SDK → Manage**. In the **App Configuration** tab, choose pick **Private** (restricted to specific domains) or **Public** and **Unlisted** checkbox distribution, choose Individual + Admin Install. In app integrations choose **Google Workspace add-on**, choose the **Apps Script project** by entering the deployment ID from step 7, and select the Docs add-on category. Script id you can find in Apps Script  → Project Settings → IDs → Script ID. Supply the add-on manifest details as prompted. Provide the list of allowed users (that you did on step 4) or domains. This keeps the listing hidden from the public catalog while giving you an install link.
